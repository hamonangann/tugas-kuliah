...
#Inisiasi The Great Library, 4 rak awal, dan jendela Tkinter
lib = Library()
...
def add_rak_gui():
  """
  Fungsi untuk memanggil metode add_rak
  Terdapat 2 entry yang harus diisi nama dan jenis rak
  Terdapat tombol untuk eksekusi dan mencetak log
  """
  def cetak(): # Mencetak log apabila tombol ditekan
    print(lib.add_rak(nama.get(), tipe.get()))
    
  master = tk.Toplevel(main_window)
...
def add_buku_gui():
  """
  Fungsi untuk memanggil metode add_buku
  Terdapat 7 entry yang harus diisi nama rak, nama buku, tahun terbit, penulis, penerbit, jenis buku, dan atribut
  Terdapat tombol untuk eksekusi dan mencetak log
  """
  def cetak():
...
def search():
  """
  Fungsi untuk memanggil metode search
  Terdapat entry untuk mengisi buku yang ingin dicar
  Terdapat tombol untuk eksekusi dan mencetak log
  """
  def cetak():
...
def daftar():
  """
  Fungsi untuk memanggil metode list_buku
  Akan tercetak daftar buku dalam setiap rak
  """
  print(lib.list_buku())

class set_gui:
    """
    Ini adalah kelas untuk GUI
    Terdapat 4 fitur yang dapat digunakan dengan menekan tombol, yaitu
    - ADD BUKU (menambah buku baru)
    - ADD RAK (menambah rak baru)
    - SEARCH (mencari buku dan menampilkan deskripsinya)
    - LIST (menampilkan daftar buku)
    """
    def __init__(self):    
      tk.Label(main_window, text="Selamat datang di The Great Library. Silakan pilih Menu yang Tersedia.").grid(row=1)
      tombol_rak = tk.Button(main_window, text="ADD BUKU", command=add_buku_gui)
      tombol_rak.grid(row=2)
      tombol_buku = tk.Button(main_window, text="ADD RAK", command=add_rak_gui)
      tombol_buku.grid(row=3)
      tombol_search = tk.Button(main_window, text="SEARCH", command=search)
      tombol_search.grid(row=4)
      tombol_list = tk.Button(main_window, text="LIST", command=daftar)
      tombol_list.grid(row=5)
